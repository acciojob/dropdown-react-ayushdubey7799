...
const states = [
    ...
    let currentState = states.filter((element) => element.name == "Madhya Pradesh");
	let currentCity = currentState[0].city.filter((element) => element.name == "Indore");
	let currentLandmark = currentCity[0].landmarks.filter((element) => element.name == "Mhow");
...
function App() {
    ...
    const [landmark,setLandmark] = useState(currentLandmark);

	const handleChange = e => {
		let x = states.filter((state) => state.name == e.target.value);
		setState(x);
		setCity({});
		setLandmark({});
	} 
	return (
		<div id="main">
			<div>
				...
				<div id="state-name">{state[0].name}</div>
				<div id="state-description">{state[0].description}</div>
				<SelectCity setCity={setCity} setLandmark = {setLandmark} state = {state} />

				<div id="city-name">{city[0]?city[0].name:""}</div>
                <div id="city-description">{city[0]?city[0].description:""}</div>

				<SelectLandmark setLandmark={setLandmark} city = {city}/>
				<div id="landmark-name">{landmark[0]?landmark[0].name:""}</div>
                <div id="landmark-description">{landmark[0]?landmark[0].description:""}</div>
			</div>
		</div>
	);
}
